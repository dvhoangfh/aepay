<template>
  <div class="content-page pricing-page">
    <div class="container" style="padding-top: 0">
      <div data-v-14a60c20="" class="btn-checkout-mobile btn btn-blue d-none">Check out</div>
      <div class="detail-plan" style="margin-top: 0; margin-bottom: 0">
        <div class="left">
          ...
          <template v-else>
            ...
            <div v-if="settings.is_enable_wordpress_stripe" class="cc-button-container" @click="onCheckoutWp('stripe')">
...
export default {
  name: "GgIframePackage",
  setup() {
    const settings = JSON.parse(Settings)
    const paypal_unavailable = ref(false)
    const voucher_success = ref(false)
    const voucher_error = ref(false)
    ...
    const mountPaypalButton = () => {
      planId.value = settings.plan_id
      if (buttons.value && buttons.value.close) {
        buttons.value.close();
      }
      buttons.value = paypal.Buttons({
        style: {
          shape: 'rect',
          color: 'gold',
          layout: 'vertical',
        ...
        createSubscription: async (data, actions) => {
          ...
          console.log('settings.plan_id', settings.plan_id)
          return actions.subscription.create({
            plan_id: planId.value,
            subscriber: {
              name: {
                given_name: userId.value,
                surname: userEmail.value,
              },
        ...
        onError: () => {
          return (err) => {
            paypal_unavailable.value = true
          }
        },
      })
      buttons.value.render('#paypal-button-container');
    ...
    const choosePlan = () => {
      ...
      if (typeof eventType !== "undefined" && typeof eventType === "string") {
        const value = JSON.parse(eventType).value
        if (value) {
          planId.value = JSON.parse(eventType).value
          mountPaypalButton()
        }
      }
    }
    ...
    onMounted(() => {
      window.addEventListener("message", choosePlan, false)
      try {
        mountPaypalButton()
      } catch (e) {
        paypal_unavailable.value = true
